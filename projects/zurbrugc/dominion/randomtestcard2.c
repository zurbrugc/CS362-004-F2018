...
** Date: 11/10/2018
** Description: Conducts random tests of the village card
*/
...
int main() {
	...
	for (int i = 0; i < 5555; i++) {
		...
		j = rand();
		beforeGame->numActions = j;
		afterGame->numActions = j;

		//any unlikely but important game states to check?
		//check game state where no cards can be drawn
			
		//every 503 instances, make sure the player has less than three cards to draw
		if (i % 503 == 72) {
			beforeGame->deckCount[beforeGame->whoseTurn] = 0;
			beforeGame->discardCount[beforeGame->whoseTurn] = 0;
			afterGame->deckCount[afterGame->whoseTurn] = 0;
			afterGame->discardCount[afterGame->whoseTurn] = 0;
		}

		//set playPos to card
		playPos = (rand() % deckLimit);
		beforeGame->hand[activePlayer][playPos] = village;
		afterGame->hand[activePlayer][playPos] = village;
