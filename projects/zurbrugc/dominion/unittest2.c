...
void checkOtherPlayers(struct gameState *testGame, struct gameState *originalGame) {
	for (int playercounter = 1; playercounter < numPlayer; playercounter++) {
		...
		for (int handcounter = 0; handcounter < MAX_HAND; handcounter++) {
			if (testGame->hand[playercounter][handcounter] != orignalGame->hand[playercounter][handcounter])
				printf("FAIL: Hand position %d not the same\n", handcounter);
		}

		//check player's hand count
		if (int testGame->handCount[playercounter] == originalGame->handCount[playercounter])
		...
		for (discardcounter = 0; discardcounter < MAX_DECK; discardcounter++) {
...
int main() {
	...
	for (int deckcounter = 0; deckcounter < MAX_DECK; deckcounter++) {
		if (testGame1->deck[0][deckcounter] != G->deck[0][deckcounter])
			printf("FAIL: Deck position %d not the same\n", deckcounter);
		if (deckcounter == 9)	//skip the one that should be different
			deckcounter++;
	}
	...
	//check player's discard
	if (testGame3->discard[0][0] == G->discard[0][0])
